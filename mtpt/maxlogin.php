...
if ($update)
	$msg = "<div><div colspan=6><b>".htmlspecialchars($update)." Successful!</b></div></div>\n";
$res = sql_query("SELECT * FROM  loginattempts ORDER BY $orderby DESC $limit") or sqlerr(__FILE__,__LINE__);
if (mysql_num_rows($res) == 0)
	  print("<div><div colspan=2><b>Nothing found</b></div></div>\n");
else
{  
  print("<div><div class=colhead><a href=?order=id>".$lang_maxlogin['text_id']."</a></div><div class=colhead align=left><a href=?order=ip>".$lang_maxlogin['text_ip']."</a></div><div class=colhead align=left><a href=?order=added>".$lang_maxlogin['text_action_time']."</a></div>".
    "<div class=colhead align=left><a href=?order=attempts>".$lang_maxlogin['text_attempts_time']."</a></div><div class=colhead align=left><a href=?order=type>".$lang_maxlogin['text_attempts_type']."</a></div><div class=colhead align=left><a href=?order=status>".$lang_maxlogin['text_status']."</a></div></div>\n");

  while ($arr = mysql_fetch_assoc($res))
  {
  	$r2 = sql_query("SELECT id,username FROM users WHERE ip=".sqlesc($arr[ip])) or sqlerr(__FILE__,__LINE__);
  	$a2 = mysql_fetch_assoc($r2);	
 	  print("<div><div align=>$arr[id]</div><div align=left>$arr[ip] " . ($a2[id] ? get_username($a2['id']) : "" ) . "</div><div align=left>$arr[added]</div><div align=left>$arr[attempts]</div><div align=left>".($arr[type] == "recover" ? $lang_maxlogin['type_recover'] : $lang_maxlogin['type_login'])."</div><div align=left>".($arr[banned] == "yes" ? "<font color=red><b>".$lang_maxlogin['status_banned']."</b></font> <a href=maxlogin.php?action=unban&id=$arr[id]><font color=green>[<b>".$lang_maxlogin['action_unban']."</b>]</font></a>" : "<font color=green><b>".$lang_maxlogin['status_notbanned']."</b></font> <a href=maxlogin.php?action=ban&id=$arr[id]><font color=red>[<b>".$lang_maxlogin['action_ban']."</b>]</font></a>")."  <a OnClick=\"return confirm('".$lang_maxlogin['text_notice']."');\" href=maxlogin.php?action=delete&id=$arr[id]>[<b>".$lang_maxlogin['action_delete']."</b></a>] <a href=maxlogin.php?action=edit&id=$arr[id]><font color=blue>[<b>".$lang_maxlogin['action_edit']."</b></a>]</font></div></div>\n");
  }
...
if ($countrows > $perpage)
	echo '<div><div colspan=2>'.$pagerbottom.'</div></div>';
searchform();
stdfoot();
}elseif ($action == 'ban') {
	check($id);
	...
	print("<div border=1 cellspacing=0 cellpadding=5 width=100%>\n");
	print("<div><div><p>".$lang_maxlogin['text_ip']."：<b>".htmlspecialchars($a[ip])."</b></p>");
	print("<p>".$lang_maxlogin['text_action_time'].": <b>".htmlspecialchars($a[added])."</b></p></div></div>");
	...
	print("<div><div>".$lang_maxlogin['text_attempts_time']."<input type='text' size='33' name='attempts' value='$a[attempts]'>");
	print("<div><div>".$lang_maxlogin['text_attempts_type']."<select name='type'><option value='login' ".($a["type"] == "login" ? "selected" : "").">".$lang_maxlogin['type_login']."</option><option value='recover' ".($a["type"] == "recover" ? "selected" : "").">".$lang_maxlogin['type_recover']."</option></select></div></div>");	
	print("<div><div>".$lang_maxlogin['text_status']."<select name='banned'><option value='yes' ".($a["banned"] == "yes" ? "selected" : "").">".$lang_maxlogin['status_banned']."</option><option value='no' ".($a["banned"] == "no" ? "selected" : "").">".$lang_maxlogin['status_notbanned']."</option></select></div></div>");	
	print("<div><div><input type='submit' name='submit' value='".$lang_maxlogin['submit_save']."' class=btn></div></div>");
	print("</div>");
	stdfoot();
	...
	if (mysql_num_rows($search) == 0)
	  print("<div><div colspan=2><b>".$lang_maxlogin['text_nothing']."</b></div></div>\n");
	else
		{  
 			print("<div><div class=colhead><a href=?order=id>".$lang_maxlogin['text_id']."</a></div><div class=colhead align=left><a href=?order=ip>".$lang_maxlogin['text_ip']."</a></div><div class=colhead align=left><a href=?order=added>".$lang_maxlogin['text_action_time']."</a></div>".
    		"<div class=colhead align=left><a href=?order=attempts>".$lang_maxlogin['text_attempts_time']."</a></div><div class=colhead align=left><a href=?order=type>".$lang_maxlogin['text_attempts_type']."</a></div><div class=colhead align=left><a href=?order=status>".$lang_maxlogin['text_status']."</a></div></div>\n");
