...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$query1 = sql_query("SELECT userid FROM forummods GROUP by userid");
		...
		{	sql_query("UPDATE users SET seedbonus = seedbonus + $point WHERE id=$dat[userid]");
			if(mysql_num_rows(sql_query("SELECT * FROM users WHERE id=$dat[userid]"))>0)sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[userid], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！原因是:{$reason_2}')") or sqlerr(__FILE__,__LINE__);
			writeBonusComment($dat[userid],"萝卜增加 $point ，原因是:{$reason_2}");
		}
	}
	...
	while($dat=mysql_fetch_assoc($query3))
		{	sql_query("UPDATE users SET seedbonus = seedbonus + $point WHERE id=".$dat[id]);
			if(mysql_num_rows(sql_query("SELECT * FROM users WHERE id=".$dat[id]))>0)sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[id], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！原因是:{$reason_2}')") or sqlerr(__FILE__,__LINE__);}
			writeBonusComment($dat[userid],"萝卜增加 $point ，原因是:{$reason_2}");}
...
{	$classname="用户";
	...
	$classname=($class==10?"贵宾":($class==11?"养老族":($class==12?"保种员":($class==13?"发布员":($class==14?"管理员":($class==15?"站长":"主管"))))));
		...
		{	sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[id], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！原因是:{$reason_2}')") or sqlerr(__FILE__,__LINE__);
		writeBonusComment($dat[userid],"萝卜增加 $point ，原因是:{$reason_2}");
		}
	}
//		write_log("25.0 bonus point is sent to everyone. -" . $CURUSER["username"] ,'normal');
...
/*	if ( $seedbonus == 0 )
	...
	//print_r( $arr );
	if ( $seedbonus > 0 )
		$bonuschangedmsg = "您的萝卜被增加了[color=red][b]".abs($seedbonus)."[/b][/color]个，";
	elseif ( $seedbonus < 0 )
		$bonuschangedmsg = "您的萝卜被减少了[color=red][b]".abs($seedbonus)."[/b][/color]个，";
	else
	...
	sql_query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES ( 0, '{$arr[id]}','$dt', '萝卜变动', '$bonuschangedmsg'  )" ) or sqlerr(__FILE__,__LINE__);
