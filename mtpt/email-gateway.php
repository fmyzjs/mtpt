...
<div class=embedded style='padding-left: 10px'><font size=3><b>Send e-mail to <?php echo $username;?></b></font></div>
</div></div></p>
<div border=1 cellspacing=0 cellpadding=5>
<form method=post action=email-gateway.php?id=<?php echo $id?>>
<div><div class=rowhead>Your name</div><div><input type=text name=from size=80></div></div>
<div><div class=rowhead>Your e-mail</div><div><input type=text name=from_email size=80></div></div>
<div><div class=rowhead>Subject</div><div><input type=text name=subject size=80></div></div>
<div><div class=rowhead>Message</div><div><textarea name=message cols=80 rows=20></textarea></div></div>
<div><div colspan=2 align=center><input type=submit value="Send" class=btn></div></div>
</form>
</div>
<p>
